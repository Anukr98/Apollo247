...
const initPayment = function(orderAutoId, amount) {
  return new Promise((resolve, reject) => {
    ...
    checksum.genchecksum(paymentObj, config.PAYTM_MERCHANT_KEY, (err, result) => {
      if (err) {
        ...
        paymentObj.CHECKSUMHASH = result.CHECKSUMHASH;
...
const responsePayment = function(paymentObject) {
  return new Promise((resolve, reject) => {
    if (
      checksum.verifychecksum(paymentObject, config.PAYTM_MERCHANT_KEY, paymentObject.CHECKSUMHASH)
    ) {
