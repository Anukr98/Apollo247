...
import {
  ...
  UpdatePatient,
} from "graphql/types/UpdatePatient";
    ...
    <div className={classes.profileBox} data-cypress="PatientProfile">
      ...
      <div className={classes.boxContent}>
        ...
        <div className={classes.userInfo}>
          ...
          ${(patient.dateOfBirth &&
            format(parseISO(patient.dateOfBirth), "dd MMMM yyyy")) ||
            ""}`}
        </div>
        <AphSelect
...
export const ExistingProfile: React.FC<ExistingProfileProps> = (props) => {
  ...
  const multiplePrimaryUsers =
    patients.filter((x) => x.relation === Relation.ME).length > 1;
  ...
  const disabled =
    patients.some(isPatientInvalid) || primaryUserErrorMessage.length > 0;
    ...
    <div className={classes.signUpPop} data-cypress="ExistingProfile">
      ...
      <div className={classes.customScrollBar}>
        <div className={classes.signinGroup}>
          ...
          {patients.length > 1 ? (
            <p>
              We have found {patients.length} accounts registered with this
              mobile number. Please tell us who is who? :)
            </p>
          ) : null}
            ...
            <AphButton
              ...
              onClick={() => {
                ...
                Promise.all(updatePatientPromises)
                  .then(() => {
                    const mePatientId = patients.filter(
                      (x) => x.relation === Relation.ME
                    );
                    if (
                      mePatientId &&
