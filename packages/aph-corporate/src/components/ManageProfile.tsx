...
export const ManageProfile: React.FC = (props) => {
  ...
  useEffect(() => {
    if (allCurrentPatients) {
      const isSomePatientMissingRelation = allCurrentPatients.some((p) =>
        _isEmpty(p.relation)
      );
  ...
  const defaultNewProfile = allCurrentPatients
    ? currentPatient || allCurrentPatients[0]
    : null;
      ...
      {({ protectWithLoginPopup, isProtected }) => (
        <div className={classes.signUpBar}>
          <div
            ...
            ref={mascotRef}
            onClick={() =>
              isProtected ? protectWithLoginPopup() : setIsPopoverOpen(true)
            }
            title={"Need Help?"}
          >
            ...
            {isSigningIn && (
              <CircularProgress
                style={{ position: "absolute", top: 17, left: 17 }}
              />
            )}
          </div>
          <Popover
