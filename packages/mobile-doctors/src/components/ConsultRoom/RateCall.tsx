  ...
  const renderMultiSelectQuestions = () => {
    return (
      <View style={{ display: hasFeedbackIssue ? 'flex' : 'none' }}>
        {renderCalltypeTab()}
        {feedbackResponseType === string.case_sheet.audio
          ? renderAudioQuestions()
            ...
            <ScrollView bounces={false} keyboardShouldPersistTaps="handled">
              ...
              {rating !== 0 && (
                <Text style={styles.thankyouTxt}>
                  {hasFeedbackIssue
                    ? string.case_sheet.what_went_wrong
                    : string.case_sheet.thankYou}
                </Text>
              )}
              {rating !== 0 && (
                <Text style={styles.thankyouSubtitle}>
                  {hasFeedbackIssue
                    ? string.case_sheet.let_us_know
                    : string.case_sheet.thankyou_subtitle}
                </Text>
              )}
