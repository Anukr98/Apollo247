import {
  ...
  export const SearchMedicineCard: React.FC<SearchMedicineCardProps> = (props) => {
    ...
    const renderAddToCartView = () => {
      return(
        <TouchableOpacity style={{alignSelf:'center'}} activeOpacity={1} onPress={!isInStock ? onNotifyMeClicked : onPressAdd} >
          <Text style={{ ...theme.viewStyles.text('SB', 12, '#fc9916', 1, 24, 0) }}>{ !isInStock ? 'NOTIFY ME' : 'ADD TO CART'}</Text>
        </TouchableOpacity>
      );
    };
    ...
    const renderQuantityView = () => {
      return(
        <View style={{ flexDirection:'row', alignSelf:'center'}}>
         ...
         <Text style={{ ...theme.viewStyles.text('B', 14, '#fc9916', 1, 24, 0), marginHorizontal:12 }}>{quantity}</Text>
         <TouchableOpacity style={{ marginRight:16}} activeOpacity={1} onPress={onPressAddQuantity} >
            <Text style={{ ...theme.viewStyles.text('SB', 14, '#fc9916', 1, 24, 0) }}>{'+'}</Text>
         </TouchableOpacity>
        </View>
      );
