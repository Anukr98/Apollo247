  ...
  const postPharmacyCartAddItemsClickedEvent = async () => {
    try {
      ...
      shoppingCart?.serverCartItems?.forEach((item) => {
        subtotal += item?.price;
        if (item?.sellingPrice) total_discount += item?.price - item?.sellingPrice;
      });
      const pincode = shoppingCart?.addresses?.find(
        (item) => item?.id === shoppingCart?.cartAddressId
      )?.zipcode;
      const eventAttributes: CleverTapEvents[CleverTapEventName.PHARMACY_CART_ADD_ITEMS_CLICKED] = {
        ...
        circle_membership_value: shoppingCart?.pharmacyCircleAttributes?.['Circle Membership Value']
          ? shoppingCart?.pharmacyCircleAttributes?.['Circle Membership Value']
          : 0,
