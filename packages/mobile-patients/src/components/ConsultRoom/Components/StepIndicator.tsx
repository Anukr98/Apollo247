...
import { StepIndicatorProps } from '@aph/mobile-patients/src/components/ConsultRoom/Components/StepIndicatorProps';
...
const StepIndicator = ({
  ...
  const borderRadiusAnim = React.useRef(new Animated.Value(customStyles.stepIndicatorSize / 2))
    .current;
  ...
  const renderStepIndicator = () => {
    ...
    for (let position = 0; position < stepCount; position++) {
      steps.push(
        <TouchableWithoutFeedback key={position} onPress={() => stepPressed(position)}>
          <View
            style={[
        ...
        >
          <View style={styles.stepLabelItem}>
            {renderLabel ? (
              ...
              <Text style={[styles.stepLabel, selectedStepLabelStyle]}>{label}</Text>
            )}
          </View>
        </TouchableWithoutFeedback>
      );
