...
export interface BannerSectionProps extends NavigationScreenProps<{}> {}
...
export const BannerSection: React.FC<BannerSectionProps> = (props) => {
      ...
      return (
        <View style={{ marginBottom: 10 }}>
          {showProxyBannerContainer ? (
            <ImageNative
              resizeMode="stretch"
              style={[styles.imageContainerStyle, { minHeight: imgHeight }]}
              source={{
                uri:
                  productsThumbnailUrl(medicineHomeBannerData[0]?.image) +
                  '?imwidth=' +
                  Math.floor(winWidth),
                headers: {
                  'User-Agent': userAgent,
          ...
          <View style={styles.bannerContainer}>
            {medicineHomeBannerData.map((_, index) =>
  ...
  return <View>{medicineHomeBannerData && renderBanners()}</View>;
};
