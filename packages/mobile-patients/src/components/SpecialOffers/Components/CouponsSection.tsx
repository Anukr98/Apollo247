...
import { filterHtmlContent, productsThumbnailUrl } from '@aph/mobile-patients/src/helpers/helperFunctions';
import HTML from 'react-native-render-html';
import { SpecialOffersCouponsData } from '@aph/mobile-patients/src/helpers/apiCalls';
import string from '@aph/mobile-patients/src/strings/strings.json';
import moment from 'moment';
import { UpOrange, DownOrange } from '../../ui/Icons';
import { YellowTickIcon, CopyBlue } from '../../ui/Icons';
...
export const CouponsSection = (props: { offersdata: SpecialOffersCouponsData[] }) => {
                ...
                <View style={styles.offerContainer}>
                    <View style={styles.singleOffer}>
                        <View style={{flexDirection: 'row'}}>
                            <View style={{paddingRight: 10, paddingLeft: 15, paddingTop: 5}}>
                            <ImageNative
                                source={{ uri: imgUrl }}
                                style={{
                                    height: 70,
                            ...
                            <View style={{flexDirection: 'column', flex: 1, }}>          
                                <Text 
                                style={styles.headerStyle}>{item.header}</Text>
                                <View style={{flexDirection: 'row', flex: 1, }}>
                                    <Text 
                                    style={[styles.subHeaderStyle, {alignSelf: 'flex-start'}]}>Valid till - </Text>
                                    <Text style={[styles.subHeaderStyle, {
                                        paddingRight: 21,
                                        fontWeight: "500",
                                        alignSelf: 'flex-start'}]}>{validTillDate}</Text>
                                    <Text style={[styles.subHeaderStyle,{paddingRight: 21}]}> |  </Text>
                                    {couponCodePresent ? (<View style={{flexDirection: 'row'}}>
                                        <Text 
                                    style={styles.subHeaderStyle}>
                                        Use Code - 
                                    </Text>
                                    <Text 
                                    style={[styles.subHeaderStyle, { fontWeight: "500"}]}>
                                        {item.couponCode}
                                    </Text></View>) : (<Text 
                                    style={styles.subHeaderStyle}>
                                ...
                                <View style={{flexDirection: 'row', flex: 1, justifyContent: 'space-between'}}>
                                    <TouchableOpacity onPress={() => updateKnowMore(index)}>
                                        <Text 
                                        style={[styles.bottomHeaderStyle,  {alignSelf: 'flex-start'}]}>
                                            {item.knowMoreOption? `Know Less ` : `Know More `}
                                            {item.knowMoreOption? <UpOrange style={styles.arrowIconStyles} />  : 
                                            <DownOrange style={styles.arrowIconStyles} /> } 
                                        </Text>
                                    </TouchableOpacity>
                                    {couponCodePresent && (<TouchableOpacity onPress={() => copyToClipboard(item.couponCode)}>
                                    <View style={{flexDirection: 'row', paddingRight: 12}}><Text 
                                    style={[styles.bottomHeaderStyle, {alignSelf: 'flex-end'}]}>
                                        {'Copy Code'}
                                    </Text>
                                    {coupon === item.couponCode ? <YellowTickIcon style={[styles.arrowIconStyles, {height: 8, width: 8}]}/> :
                                    <CopyBlue style={[styles.arrowIconStyles, {height: 8, width: 8}]}/>}
                                    </View>
                                    </TouchableOpacity>
                                    )}
                                </View>
                    ...
                    {item.knowMoreOption && (
                        <View style={styles.singleOffer}>
                            <Text 
                            style={styles.couponKnowMoreText}>
                                {string.specialOffersScreen.couponKnowMore}
                            </Text>
                            <View style={{paddingLeft: 15}}>
                            <HTML
                               html={filterHtmlContent(item.knowMore)}
                               baseFontStyle={styles.htmlContent}
                            />
                            </View>
                            <TouchableOpacity onPress={() => {
                                ...
                                <Text style={[styles.couponKnowMoreText, styles.termsConditions]}>{string.specialOffersScreen.readTermsAndConditions}</Text>
                            </TouchableOpacity>
                        </View>
                    )}
                </View>
    ...
    const renderViewAllOffersButton = () => {
        return (
            <View style={styles.buttonContainerStyle}>
                <TouchableOpacity
                onPress={() => {visibleOffers<couponOffersData.length ? setVisibleOffers(couponOffersData.length) :
                    setVisibleOffers(2)}}
                style={{flexDirection: 'row', justifyContent: 'center'}}>
                    ...
                    {visibleOffers<couponOffersData.length ?
                    <DownOrange style={{height: 10, width: 10, paddingTop: 3, alignSelf: 'center'}} /> :
                    <UpOrange style={{height: 10, width: 10, paddingTop: 3, alignSelf: 'center'}} />}
                </TouchableOpacity>
            </View>
        );
    };
        ...
        >
            ...
            >
              <Text style={{ color: '#02475B' }}>X</Text>
                ...
                <HTML
                    html={filterHtmlContent(popupText)}
                />
            </View>
            </Overlay>
        );  
    ...
    return (
        <View style={styles.container}>
            ...
            data={visibleOffers<couponOffersData.length ? couponOffersData.slice(0, visibleOffers) : couponOffersData}
