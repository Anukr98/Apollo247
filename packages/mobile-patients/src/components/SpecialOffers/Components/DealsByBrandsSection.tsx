...
import { addPharmaItemToCart, getIsMedicine, getMaxQtyForMedicineItem, productsThumbnailUrl } from '@aph/mobile-patients/src/helpers/helperFunctions';
import { SectionHeaderComponent } from '../../ui/SectionHeader';
import { getSpecialOffersPageBrandsProducts, MedicineProduct } from '@aph/mobile-patients/src/helpers/apiCalls';
...
export interface DealsByBrandsProps
...
export const DealsByBrandsSection: React.FC<DealsByBrandsProps> = (props) => {
    ...
    useEffect(() => {
      ...
      const fetchDataForBrandProducts = async () => {
        try {
            ...
            const productsResponse = await getSpecialOffersPageBrandsProducts(selectedBrandID.toString(), discountPercentage);
    ...
    function getDiscountPercent(price: number, specialPrice: number) {
        return (((price - specialPrice) / price) * 100).toFixed(1);
    }
        ...
        return (
            <View style={styles.categoryContainer}>
                <TouchableOpacity
                    ...
                    onPress={() => discountPresent ? onPressBrand(id, discountValueArray[0], discountValueArray[1]) : onPressBrand(id, defaultMinDiscount, defaultMaxDiscount)}
                    >
                <View style={styles.categoryBoxStyles}>
                   ...
                   <Text style={id === selectedBrandID ? styles.selectedIdPromotionStyle : styles.promotionStyle}>{promotionalMessage}</Text>
                   </View>
                   </View>
                   </TouchableOpacity>
            </View>
            ...
            <View style={styles.hotSellersContainer}>
                <View style={styles.hotSellersBoxStyles}>
                    <TouchableOpacity onPress={() => onPressProduct(item.url_key, item.sku)}>
                       {(
                           <View style={styles.squareContainer}>
                               {loading ? (<View><ActivityIndicator color='#FC9916' size="large" /></View>) : 
                           (<ImageNative
                   ...
                   <View style={{paddingBottom: 5, height: 20}}>
                    ...
                    <View style={{flexDirection: 'row', justifyContent: 'center', paddingBottom: 5, height: 20}} >  
                      {specialPrice && (<Text style={styles.priceCancelStyle}>{`${'\u20B9'}${item.price}`}</Text>)}
                      {specialPrice && (<Text style={styles.discountStyle}>{`${getDiscountPercent(item.price, item.special_price)}%off`}</Text>)}
                    </View>
                    </TouchableOpacity>
