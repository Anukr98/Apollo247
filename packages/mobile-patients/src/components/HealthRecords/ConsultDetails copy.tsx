...
import { getPlaceInfoByPincode } from '../../helpers/apiCalls';
...
export const ConsultDetails: React.FC<ConsultDetailsProps> = (props) => {
  ...
  const onAddTestsToCart = async () => {
    ...
    const getPincode = g(diagnosticLocation || locationDetails || location, 'pincode' || '');
    ...
    if (!!getPincode) {
      ...
      client
        .query<getPincodeServiceability, getPincodeServiceabilityVariables>({
          ...
          variables: {
            pincode: parseInt(getPincode, 10),
          },
          fetchPolicy: 'no-cache',
        })
        .then(({ data }) => {
          ...
          if (serviceableData && serviceableData?.cityName != '') {
            ...
            updatePlaceInfoByPincode(getPincode);
            addTestsToCart(testPrescription, client, obj.cityId)
            ...
            Alert.alert('Uh oh.. :(', `Unfortunately, we are not servicing in this area`);
            console.log('not servicing in this area');
          }
          setLoading!(false);
        })
        .catch((e) => {
