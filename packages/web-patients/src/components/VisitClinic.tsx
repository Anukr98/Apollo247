...
} from 'graphql/types/BookAppointment';
...
export const VisitClinic: React.FC<VisitClinicProps> = props => {
  ...
  const { currentLocation, currentLat, currentLong } = useContext(
    LocationContext
  );
  ...
  const availableSlots =
    (data && data.getDoctorPhysicalAvailableSlots.availableSlots) || [];
  availableSlots.map(slot => {
    ...
    if (slotTime > currentTime) {
      ...
      else if (slotTime >= morningTime && slotTime < afternoonTime)
  ...
  const defaultClinicAddress =
    clinics.length > 0 && clinics[0] && clinics[0].facility
      ? `${
          ...
          clinics[0].facility.streetLine2
            ? `${clinics[0].facility.streetLine2}`
            : ''
        } ${
          clinics[0].facility.streetLine3
            ? `${clinics[0].facility.streetLine3}`
            : ''
        }`
      : '';
          ...
          <Grid container spacing={2}>
            ...
            <Grid item sm={6}>
              <div className={`${classes.consultGroup} ${classes.timeSlots}`}>
                <AphSelect
                  value={
                    clinicSelected === '' ? defaultClinicId : clinicSelected
                  }
                  onChange={e => {
        ...
        >
          {mutate => (
            <AphButton
              ...
              className={
                disableSubmit || mutationLoading || isDialogOpen
