...
export const CasesheetView: React.FC<savingProps> = (props) => {
  ...
  const removedMedicineHtml = removedMedicinePrescription &&
  ...
  const medicineHtml = medicinePrescription &&
    medicinePrescription!.map(
      (
        ...
        const whenString = prescription!.medicineToBeTaken!.length > 0
          ? toBeTaken(prescription!.medicineToBeTaken)
            .join(", ")
            .toLowerCase()
          : "";
        const unitHtmls =
          prescription && medUnitObject &&
            medUnitObject[prescription!.medicineUnit!]
            ? medUnitObject[prescription!.medicineUnit!].value
            : prescription!.medicineUnit!.toLowerCase();
        const isInDuration = prescription &&
          ...
          ? ""
          : "in the ";
        let timesString = prescription!.medicineTimings!.length > 0
          ? isInDuration +
            prescription!
        ...
        if (
          ...
          if (
            dosageTimingArray && dosageTimingArray[0] &&
            dosageTimingArray[0] !== "0"
          ) {
            customTimingArray.push(dosageTimingArray[0] + unitHtmls);
          }
          if (
            dosageTimingArray && dosageTimingArray[1] &&
            dosageTimingArray[1] !== "0"
          ) {
            customTimingArray.push(dosageTimingArray[1] + unitHtmls);
          }
          if (
            dosageTimingArray && dosageTimingArray[2] &&
            dosageTimingArray[2] !== "0"
          ) {
            customTimingArray.push(dosageTimingArray[2] + unitHtmls);
          }
          if (
            dosageTimingArray && dosageTimingArray[3] &&
            dosageTimingArray[3] !== "0"
          ) {
  ...
  const generateSymptomsHtml = (
    symptom: GetCaseSheet_getCaseSheet_caseSheetDetails_symptoms,
  ) => {
    {
    ...
    symptom && symptom.howOften &&
      symptomArray.push(`How often: ${symptom.howOften}`);
    symptom && symptom.severity &&
      symptomArray.push(`Severity: ${symptom.severity}`);
    symptom && symptom.details &&
      symptomArray.push(`Details: ${symptom.details}`);
    ...
    <div className={classes.root} id={"prescriptionWrapper"}>
      ...
      <div className={classes.prescriptionPreview}>
            ...
            ? (
              <div className={classes.doctorInformation}>
                ...
                {currentDoctor.qualification && (
                  <p
                    className={`${classes.specialty} ${classes.qualification}`}
                  >
                    {currentDoctor.qualification}
                  </p>
                )}
        ...
        <div className={classes.pageContent}>
            ...
            <div className={classes.consultInfo}>
                  ...
                  <div className={classes.infoRow}>
                    ...
                    <div className={classes.labelContent}>
                      <div className={classes.patientName}>
                        {`${patientDetails.firstName}  ${patientDetails.lastName} | ${patientDetails.gender} | ${
              ...
              <div className={classes.accountDetails}>
                {appointmentInfo && appointmentInfo.appointmentDateTime
                  ? (
                    <div className={classes.infoRow}>
                      <div className={`${classes.label} ${classes.labelRight}`}>
                        Consult Date
                      </div>
                      <div className={classes.labelContent}>
                        <div className={classes.labelBlue}>
                          {updatedDate && updatedDate !== ""
                            ? `${moment(updatedDate).format("DD/MM/YYYY")} at ${
                              moment(
                                updatedDate,
                              ).format("h:mm a")
                            }`
                            : `${moment(new Date()).format("DD/MM/YYYY")} at ${
                              moment(
                                new Date(),
                              ).format("h:mm a")
                            }`}
          ...
          {!loader &&
              ((symptoms && symptoms.length > 0) || weight || height || bp ||
                temperature)
            ? (
              <div className={classes.prescriptionSection}>
                ...
                <div className={classes.chiefComplaints}>
                  ...
                  {vitalsArr.length > 0
                    ? (
                      <div className={classes.complaintsInfoRow}>
                        <div
                          className={`${classes.complaintsLabel} ${classes.vitalLabel}`}
                        >
                          VITALS <span className={classes.subInfo}>
                            (as declared by patient)
                          </span>
                        </div>
                        <div className={classes.labelContent}>
                          <div
