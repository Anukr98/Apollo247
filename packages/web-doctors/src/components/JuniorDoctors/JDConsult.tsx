...
export const JDConsult: React.FC<ConsultProps> = (props) => {
  ...
  const [downgradeToAudio, setDowngradeToAudio] = React.useState<boolean>(
    false,
  );
  ...
  const isPaused = () => {
    if (!callerAudio && !callerVideo && getCookieValue() === "videocall") {
      ...
      !callerVideo && getCookieValue() === "videocall"
    ) {
  ...
  return (
    <div className={classes.consult}>
      <div>
        <div
          className={props.showVideoChat || !subscribeToVideo
            ? "chatVideo"
            : ""}
        >
          {!props.showVideoChat && (
            <div className={classes.timerCls}>
              ...
              <p className={classes.audioVideoState}>
                {checkDowngradeToAudio()}
              </p>
          ...
          {props.isCall && (
            <OTSession
              ...
              eventHandlers={{
                connectionDestroyed: (event: any) => {
                  ...
                  if (event.reason === "networkDisconnected") {
                    props.setSessionError(
                      {
                        "message":
                          "Call was disconnected due to Network problems on the patient end.",
                      },
                    );
                  } else {
                    props.setSessionError(
                      { "message": "Patient left the call." },
                    );
                  }
                },
                streamPropertyChanged: (event: any) => {
                  const subscribers = event.target.getSubscribersForStream(
                    event.stream,
                  );
                ...
                error: (error: any) => {
                  console.log(
                    `There was an error with the sessionEventHandlers: ${
            ...
            >
              <OTPublisher
                ...
                onError={(error: any) => {
                  ...
                  if (error.name === "OT_USER_MEDIA_ACCESS_DENIED") {
                    ...
                    props.setPublisherError(
                      { "message": "Audio/Video device is not connected." },
                    );
                  } else if (
                    error.name === "OT_CHROME_MICROPHONE_ACQUISITION_ERROR"
                  ) {
                    props.setPublisherError(
                      { "message": "Audio device is not connected." },
                    );
                  } else {
                ...
                eventHandlers={{
                  error: (error: any) => {
                    console.log(
                      `There was an error with the publisherEventHandlers: ${
                        JSON.stringify(error)
                      }`,
                    );
                    props.setPublisherError(error);
              ...
              >
                ...
                <OTStreams>
                  <OTSubscriber
                    className={!props.showVideoChat ? classes.subscriber
                    : classes.minSubscriber}
                    retry={isRetry}
                    onError={(error: any) => {
                    ...
                    eventHandlers={{
                      ...
                      error: (error: any) => {
                        console.log(
                          `There was an error with the subscriberEventHandlers: ${
                            JSON.stringify(
                              error,
                            )
                          }`,
