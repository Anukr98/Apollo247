...
export const CallPopover: React.FC<CallPopoverProps> = (props) => {
  ...
  const myDate = new Date(); // Set this to your date in whichever timezone.
  var today = moment();
  console.log(moment(today).add(1, 'days').toISOString());
  const rescheduleConsultAction = () => {
    ...
    if (reason === 'Other' && isEmpty(otherTextValue)) {
      ...
      client
        .mutate<InitiateRescheduleAppointment, InitiateRescheduleAppointmentVariables>({
          ...
          variables: {
            RescheduleAppointmentInput: {
              ...
              rescheduledDateTime: moment(today).add(1, 'days').toISOString(),
              autoSelectSlot: 0,
            },
          },
        })
        .then((_data) => {
          //setIsLoading(false);
          const isCancelled = (_data && _data!.data &&
            _data!.data!.initiateRescheduleAppointment &&
            _data!.data!.initiateRescheduleAppointment.cancelled) ? true : false
