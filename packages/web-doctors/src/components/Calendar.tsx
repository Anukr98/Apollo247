...
import {
  ...
  GetDoctorDetails_getDoctorDetails_consultHours as consultHours,
...
const dataAdapter = (
  ...
  let selectedDay = selectedDate!.toLocaleDateString('en-US', { weekday: 'long' }).toUpperCase();
  // let consultHours: consultHours;
  // if (doctorData) {
  //   consultHours = doctorData!.consultHours! as consultHours;
  // }
  ...
  const consultDuration =
    filteredDay && filteredDay[0]!.consultDuration ? filteredDay[0]!.consultDuration : 15;
  ...
  const adaptedList: Appointment[] = appointments.map(
    (appointment: GetDoctorAppointments_getDoctorAppointments_appointmentsHistory | null) => {
      ...
      const endTime = getTime(addMinutes(startTime!, consultDuration!));
      let symptoms = null;
      if (
        caseSheet &&
        caseSheet.length > 0 &&
