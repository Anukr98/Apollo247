...
export const MedicinePrescription: React.FC = () => {
    ...
    axios
      ...
      .then(result => {
        if (
          ...
          result.data.productdp[0].PharmaOverview.length > 0 &&
          result.data.productdp[0].PharmaOverview[0].Doseform &&
          gelLotionOintmentTypes.indexOf(
            result.data.productdp[0].PharmaOverview[0].Doseform
          ) > -1
        ) {
          setMedicineUnit(result.data.productdp[0].PharmaOverview[0].Doseform);
  ...
  const generateFrequency = dosageFrequency.map(
    (dosageObj: SlotsObject, index: number) => {
      return (
        <MenuItem
          key={index.toString()}
          classes={{
  ...
  const forOptionHtml = forOptions.map(
    (optionObj: SlotsObject, index: number) => {
      return (
        <MenuItem
          key={index.toString()}
          classes={{
    ...
    <div className={classes.root}>
      <Grid container spacing={1}>
          ...
          {selectedMedicinesArr!.map((_medicine: any, index: number) => {
            ...
            const unitHtmls =
              medicine.medicineUnit[
                medicine.medicineUnit.length - 1
              ].toLowerCase() === "s"
            ...
            return (
              <div style={{ position: "relative" }} key={index}>
                <Paper className={classes.medicineCard}>
                  ...
                  <h6>
                    {`${takeApplyHtml} ${
                      dosageCount && medicine.medicineFormTypes === "OTHERS"
                        ? dosageCount
                        : ""
                    } ${unitHtml} ${
                  ...
                  {medicine.medicineInstructions && (
                    <h6>{medicine.medicineInstructions}</h6>
                  )}
                </Paper>
                {caseSheetEdit && (
                  <AphButton
        ...
        {!showAddCondition &&
          caseSheetEdit &&
          favouriteMedicine &&
          favouriteMedicine.length > 0 && (
            <Grid item lg={6} xs={12}>
              <div className={classes.favmedicineHeading}>
                Favourite Medicines
              </div>
                ...
                {favouriteMedicine.map((_favMedicine: any, id, index) => {
                  ...
                  const favUnitHtmls =
                    favMedicine.medicineUnit[
                      favMedicine.medicineUnit.length - 1
                    ].toLowerCase() === "s"
                  ...
                  if (favTimesString && favTimesString !== "") {
                    favTimesString = favTimesString.replace(
                      /,(?=[^,]*$)/,
                      "and"
                    );
                  }
                  ...
                  <div className={classes.dialogContent}>
                    <Grid container spacing={2}>
                      ...
                      <Grid item lg={12} xs={12}>
                        <h6>In The</h6>
                        <div
                          className={`${classes.numberTablets} ${classes.daysOfWeek}`}
                        >
                          {daySlotsHtml}
                        </div>
