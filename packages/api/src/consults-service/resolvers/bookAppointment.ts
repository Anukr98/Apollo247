...
> = async (
  ...
  if (facilityId) {
    const doctorHospRepo = doctorsDb.getCustomRepository(
  ...
  const blockRepo = doctorsDb.getCustomRepository(
    BlockedCalendarItemRepository
  );
  ...
  if (appointmentInput.appointmentDateTime <= new Date()) {
    throw new AphError(
      AphErrorMessages.APPOINTMENT_BOOK_DATE_ERROR,
      undefined,
      {}
    );
  }
  ...
  if (patientConsults) {
    throw new AphError(
      AphErrorMessages.ANOTHER_DOCTOR_APPOINTMENT_EXIST,
      undefined,
      {}
    );
  }
  ...
  const appointmentAttrs: Omit<AppointmentBooking, "id"> = {
    ...
    appointmentDateTime: new Date(
      appointmentInput.appointmentDateTime.toISOString()
    ),
  ...
  const istDateTime = addMilliseconds(
    appointmentInput.appointmentDateTime,
    19800000
  );
