...
import {
    REFUND_STATUS, AppointmentRefunds,
    AppointmentPayments,
    Appointment
...
export const initiateRefund: refundMethod<
    RefundInput,
    Connection,
    Partial<PaytmResponse>
> = async (refundInput, consultsDb: Connection) => {
    ...
    try {
        ...
        const checksumHash = await genCheckSumPromiseWrapper(paytmBody, process.env.PAYTM_MERCHANT_KEY_CONSULTS ? process.env.PAYTM_MERCHANT_KEY_CONSULTS : '');
        ...
        const paytmResponse = await appointmentRefRepo.raiseRefundRequestWithPaytm(paytmParams, process.env.PAYTM_REFUND_URL ? process.env.PAYTM_REFUND_URL : '');
        ...
        if (paytmResult.resultInfo)
            paytmResult.resultInfo.resultStatus = resultStatus;
        return paytmResult;
    } catch (e) {
        log(
            'consultServiceLogger',
            JSON.stringify(refundInput),
            'initiateRefund()',
            e.stack,
            'true'
        );
