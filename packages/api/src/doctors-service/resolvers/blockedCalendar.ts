...
export const blockedCalendarTypeDefs = gql`
	...
	extend type Mutation {
		...
		updateBlockedCalendarItem(id: Int!, doctorId: String!, start: DateTime!, end: DateTime!): BlockedCalendarResult!
		removeBlockedCalendarItem(id: Int!): BlockedCalendarResult!
		blockMultipleCalendarItems(blockCalendarInputs: BlockMultipleItems): String
	}
`;
...
const getBlockedCalendar: Resolver<null, { doctorId: string }, DoctorsServiceContext, BlockedCalendarResult> = async (
	parent,
	{ doctorId },
	context
) => {
...
> = async (parent, { doctorId, start, end, reason }, context) => {
	...
	const rescheduleRepo = context.consultsDb.getCustomRepository(RescheduleAppointmentDetailsRepository);
...
if (overlap) throw new AphError(AphErrorMessages.BLOCKED_CALENDAR_ITEM_OVERLAPS);*/
	...
	const itemToUpdate = await bciRepo.findOneOrFail(id);
	const existingItems = (await bciRepo.find({ doctorId })).filter((item) => item.id !== itemToUpdate.id);
	const overlap = doesItemOverlap(itemToAdd, existingItems);
	if (overlap) throw new AphError(AphErrorMessages.BLOCKED_CALENDAR_ITEM_OVERLAPS);
...
const removeBlockedCalendarItem: Resolver<null, { id: string }, DoctorsServiceContext, BlockedCalendarResult> = async (
	parent,
	{ id },
	context
) => {
...
type BlockCalendarInputs = {
	blockCalendarItems: BlockMultipleItems;
};

const blockMultipleCalendarItems: Resolver<null, BlockCalendarInputs, DoctorsServiceContext, string> = async (
	parent,
	blockMultipleItems,
	context
) => {
	//checkAuth(blockMultipleItems.blockCalendarItems.doctorId, context);
	console.log(blockMultipleItems);
	/*const { bciRepo } = getRepos(context);
  const itemToAdd = bciRepo.create({ doctorId, start, end, reason });
  const existingItems = await bciRepo.find({ doctorId });
  const overlap = doesItemOverlap(itemToAdd, existingItems);
