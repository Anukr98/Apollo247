...
const insertPincodes = async (client, pincodeMaster) => {
  async.eachSeries(
    pincodeMaster,
    (element, callback) => {
      let { StateID, state, CityID, City, PinCode, LocationID, LocationName } = element;
      ...
      const str = `INSERT INTO diagnostic_itdose_pincode_hubs(state_id, state, city_id, city, area_id, area, pincode) VALUES (${StateID}, '${state}', ${CityID}, '${City}', ${LocationID}, '${LocationName}', ${PinCode})`;
      console.log(str);
      client.query(str, (err) => {
        if (err) {
          return callback(err);
...
const getPincodeMaster = async (token) => {
  const apiUrl = process.env.DIAGNOSTIC_ITDOSE_GETLOCATION_URL;
  if (!apiUrl) {
    throw new Error('add env DIAGNOSTIC_ITDOSE_GETLOCATION_URL');
  }
  let options = {
    method: 'POST',
